...
// =============================================
const DASHBOARD_VERSION = "2026-04-24-T1130 (ULTIMATE v1408)";
...
async function ensurePageLoaded(pageId) {
    console.log(`🔍 [LOADER] Checking if page is loaded: ${pageId}`);
    const pageEl = document.getElementById(pageId);
    if (!pageEl) {
        console.error(`❌ [LOADER] Element not found in DOM: #${pageId}`);
        return false;
    }
    
    // If page is not empty and not just whitespace, skip loading
    const contentLen = pageEl.innerHTML.trim().length;
    console.log(`📊 [LOADER] Page content length for ${pageId}: ${contentLen}`);
    if (contentLen > 50) {
        console.log(`✅ [LOADER] Page ${pageId} already has content. Skipping fetch.`);
        return true;
    }
    ...
    if (!modularPages.includes(pageId)) {
        console.warn(`⚠️ [LOADER] Page ${pageId} is not in modular list and is empty!`);
        return true;
    }
    ...
    try {
        ...
        console.log(`✅ [LOADER] Page ${pageId} injected successfully.`);
        
        // Post-injection setup for specific pages
        if (pageId === 'job_radar') {
            console.log('⚙️ [LOADER] Running Job Radar init...');
...
async function showPage(id) {
  console.log(`\n%c--- 🚀 [NAV START] Target: ${id} ---`, 'background: #1e293b; color: #38bdf8; font-weight: bold; padding: 4px 8px; border-radius: 4px;');
  
  // 1. Audit Body Layout
  console.group('📦 [AUDIT] Body Level Layout');
  console.log(`Viewport: ${window.innerWidth}x${window.innerHeight}`);
  document.body.childNodes.forEach(node => {
    if (node.nodeType === 1) { // Element
      const style = window.getComputedStyle(node);
      if (style.display !== 'none') {
        console.log(`Child: <${node.tagName} id="${node.id}" class="${node.className}"> Dim: ${node.offsetWidth}x${node.offsetHeight}, Display: ${style.display}, Position: ${style.position}, Z: ${style.zIndex}`);
      }
    }
  });
  console.groupEnd();
  ...
  if (mainEl) {
    ...
    console.log(`🏠 [NAV] #main state: visibility=${mainStyle.visibility}, opacity=${mainStyle.opacity}, display=${mainStyle.display}, height=${mainEl.offsetHeight}px, overflow=${mainStyle.overflowY}`);
  }
  ...
  if (!page && !topicConfig[id]) {
    console.error(`❌ [NAV] CRITICAL: Element #${id} not found in DOM and no topic mapping exists.`);
    return;
  }

  // Try Industrial Content
  const isIndustrial = renderTopicContent(id);
  if (isIndustrial) page = document.getElementById('topic_viewer');
  else if (!page && topicConfig[id]) page = document.getElementById('topic_viewer');
  ...
  if (page) { 
    ...
    // Check computed state AFTER enabling
    const computed = window.getComputedStyle(page);
    console.log(`📊 [NAV] #${page.id} COMPUTED: display=${computed.display}, visibility=${computed.visibility}, opacity=${computed.opacity}, height=${page.offsetHeight}px, pos=${computed.position}`);
    
    // Audit Siblings inside #main
    console.group('🌿 [AUDIT] #main Children');
    mainEl.childNodes.forEach(node => {
      if (node.nodeType === 1) {
        const s = window.getComputedStyle(node);
        if (s.display !== 'none') {
           console.log(`Sibling: <${node.tagName} id="${node.id}"> Dim: ${node.offsetWidth}x${node.offsetHeight}, Display: ${s.display}`);
        }
      }
    });
    console.groupEnd();
    ...
    // Init Logic
    if (id === 'schedule') { 
        console.log('📅 [NAV] Initializing Schedule...');
        await renderTimetable(); 
        console.log(`📅 [NAV] Post-Render Height of #schedule: ${page.offsetHeight}px`);
    }
    ...
    if (id === 'job_radar') { 
        console.log('📡 [NAV] Initializing Job Radar...');
        fetchJobsList(); 
    }
    if (id === 'profile_match') { 
    ...
    console.error('❌ [NAV] FATAL: Navigation failed.');
  }

  // Final visibility check
  setTimeout(() => {
    if (page) {
       console.log(`🏁 [NAV] 500ms Visibility Check: #${page.id} Height = ${page.offsetHeight}px`);
       if (page.offsetHeight === 0) {
          console.warn('⚠️ [NAV] WARNING: Page still has 0 height after 500ms! Checking for parent constraints...');
          let parent = page.parentElement;
          while(parent) {
             console.log(`   Parent <${parent.tagName} id="${parent.id}">: height=${parent.offsetHeight}px, overflow=${getComputedStyle(parent).overflow}`);
